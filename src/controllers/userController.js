...
const userLogin = async (req, res) => {
  try {
    ...
    return res.status(result.statusCode).json({
      ...
      token: result.token ? result.token : undefined,
    });
  } catch (error) {
    return res.status(500).json({ message: "failed" });
  }
...
const purchasePremium = async (req, res) => {
  try {
    ...
    const amount = 25000;
    const orderExist = await userServices.findByStatusAndId({
    ...
    if (orderExist) {
      return res
        .status(201)
        .json({ orderid: orderExist.dataValues.orderId, key_id: rzp.key_id });
    } else {
      rzp.orders.create({ amount, currency: "INR" }, async (err, order) => {
        if (err) {
          throw new Error(err);
        }
        await userServices.createOrder({
          orderId: order.id,
          status: "PENDING",
          userId: res.locals.userId,
        });
...
const updateTransactionStatus = async (req, res) => {
  try {
    ...
    await userServices.orderUpdate(
      { paymentId: payment_id, status: "SUCCESSFUL" },
      { orderId: order_id }
    );
    await userServices.userUpdate(
      { isPremiumUser: true },
