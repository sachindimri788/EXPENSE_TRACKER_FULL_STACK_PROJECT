...
dailyDateShowBtn.addEventListener("click", async () => {
  try {
    ...
    const resp = await axios.post(
      "http://localhost:4000/premium/getDailyExpense",
      {
        date: dailyDate,
      },
      {
    ...
    if (data != null) {
      for (let i = 0; i < data.length; i++) {
        ...
        content =
          content +
          `${formattedDate} , ${data[i].category}, ${data[i].description}, ${data[i].expenseAmount} \n`;
      }
...
monthlyDateShowBtn.addEventListener("click", async () => {
  try {
    ...
    const resp = await axios.post(
      "http://localhost:4000/premium/getMonthlyExpense",
      {
        date: monthlyDate,
      },
      {
